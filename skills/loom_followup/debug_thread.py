...
Usage:
    python skills/loom_followup/debug_thread.py --thread-id <id> --lead-email <email>

Example (use a thread where you KNOW they replied):
...
def check_thread(thread_id: str, lead_email: str):
    ...
    if not replied:
        ...
        print("Check the 'Fields available' output above and update has_replied() in pipeline.py.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Debug Instantly thread reply detection")
    parser.add_argument("--thread-id", required=True)
    parser.add_argument("--lead-email", required=True)

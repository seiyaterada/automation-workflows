...
2. Ask the user: **"Run in dry-run mode (preview only) or live mode (actually sends emails)?"**
   - If dry-run: run `python skills/loom_followup/pipeline.py --dry-run`
   - If live: run `python skills/loom_followup/pipeline.py`
...
4. If there were any errors, read the error message, diagnose the cause, and tell the user what went wrong and how to fix it. Common issues:
   ...
   - Instantly API field name mismatch → run `python skills/loom_followup/debug_thread.py --thread-id <id> --lead-email <email>` with a known-replied lead to inspect the raw response, then update `has_replied()` in `skills/loom_followup/pipeline.py`

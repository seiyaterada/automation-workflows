...
SHEET_ID = os.getenv("INTERESTED_LEADS_SHEET_ID")
...
def get_row_index(sheet, lead_email: str) -> Optional[int]:
    ...
    try:
        return emails.index(lead_email) + 1
    except ValueError:
        return None
...
def is_due(lead: dict) -> bool:
    ...
    if count == 0:
        ...
        if anchor is None:
            # Fallback: loom_sent_at + cumulative days already elapsed
            anchor = parse_dt(str(lead.get("loom_sent_at", "")))
            if anchor:
...
def send_reply(lead: dict, subject: str, body: str, dry_run: bool) -> Optional[str]:
    ...
    if dry_run:
        ...
        print(f"  Subject: {subject or '(inherited from thread)'}")
...
def run(dry_run: bool = False):
    ...
    if not SHEET_ID:
        sys.exit("[ERROR] INTERESTED_LEADS_SHEET_ID not set in .env")
    ...
    for lead in eligible:
        ...
        if has_replied(thread_id, email):
